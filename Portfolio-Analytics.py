...
high_prices = pd.read_csv('High Prices.csv')
low_prices = pd.read_csv('Low Prices.csv')
open_prices = pd.read_csv('Open Prices.csv')
close_prices = pd.read_csv('Close Prices.csv')
volumes = pd.read_csv('Volumes.csv')

high_prices.columns = ['date'] + equities
high_prices = high_prices.drop(['date'], axis=1)
low_prices.columns = ['date'] + equities
low_prices = low_prices.drop(['date'], axis=1)
open_prices.columns = ['date'] + equities
open_prices = open_prices.drop(['date'], axis=1)
close_prices.columns = ['date'] + equities
close_prices = close_prices.drop(['date'], axis=1)
volumes.columns = ['date'] + equities
volumes = volumes.drop(['date'], axis=1)

# get the S&P500 benchmark price data
benchmark = pd.read_csv('Benchmark Prices.csv')
benchmark.columns = ['date', 'SP500']
...
benchmark_returns = benchmark.div(benchmark.iloc[0])
